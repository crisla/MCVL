...
* Stocks * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
log using stocks_ss_ltu_prior.log,replace
tab time state
log close

// * Stocks (by age) - pre 2008
// log using stocks_ss_age_ltu.smcl,replace
// tab age state if age>19&age<61&post08==0&state!="R"&state!="A"
// log close
// translate stocks_ss_age_ltu.smcl stocks_ss_age_m.log,replace
//
...
log using stocks_ss_age_ltu_prior.log,replace
...
* Stocks by sex
log using stocks_ss_sex_ltu_prior.log,replace
tab time state if sex==1
tab time state if sex==2
log close

* Flows (by state) * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
log using flows_ss_u_ltu_prior.log,replace
tab time flow if state=="U"&length(flow)==2
log close

log using flows_ss_t_ltu_prior.log,replace
tab time flow if state=="T"&length(flow)==2
log close

log using flows_ss_p_ltu_prior.log,replace
tab time flow if state=="P"&length(flow)==2
log close

log using flows_ss_0_ltu_prior.log,replace
tab time flow if state=="0"&length(flow)==2
log close

* Flows (by state and age) * * * * * * * * * * * * * * * * * * * * * * * * * * 
* Pre 2008
// log using flows_age_ltu08.log,replace
...
//  *
//  log close
//  translate flows_age_bad.smcl flows_age_bad_prior.log,replace
*
