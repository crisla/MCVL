...
* Stocks * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
log using stocks_ss_ltu_y2.log,replace
tab time state
log close
...
log using stocks_ss_age_ltu_y2.log,replace
...
* Stocks by sex
log using stocks_ss_sex_ltu_y2.log,replace
tab time state if sex==1
tab time state if sex==2
log close

* Flows (by state) * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
log using flows_ss_u_ltu_y2.log,replace
tab time flow if state=="U"&length(flow)==2
log close

log using flows_ss_t_ltu_y2.log,replace
tab time flow if state=="T"&length(flow)==2
log close

log using flows_ss_p_ltu_y2.log,replace
tab time flow if state=="P"&length(flow)==2
log close

log using flows_ss_0_ltu_y2.log,replace
...
//  *
//  log close
//  translate flows_age_bad.smcl flows_age_bad_y2.log,replace
*
