...
* Stocks * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
log using "stocks_ss_$flabel_$freq.log",replace
tab time state
log close

// * Stocks (by age) - pre 2008
// log using stocks_ss_age_$flabel.smcl,replace
// tab age state if age>19&age<61&post08==0&state!="R"&state!="A"
// log close
// translate stocks_ss_age_$flabel.smcl stocks_ss_age_m.log,replace
//
...
log using "stocks_ss_age_$flabel_$freq.log",replace
...
* Stocks by sex
log using "stocks_ss_sex_$flabel_$freq.log",replace
tab time state if sex==1
tab time state if sex==2
log close

* Flows (by state) * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
log using "flows_ss_u_$flabel_$freq.log",replace
tab time flow if state=="U"&length(flow)==2
log close

log using "flows_ss_t_$flabel_$freq.log",replace
tab time flow if state=="T"&length(flow)==2
log close

log using "flows_ss_p_$flabel_$freq.log",replace
tab time flow if state=="P"&length(flow)==2
log close

log using "flows_ss_0_$flabel_$freq.log",replace
tab time flow if state=="0"&length(flow)==2
log close

* Flows (by state and age) * * * * * * * * * * * * * * * * * * * * * * * * * * 
* Pre 2008
// log using flows_age_$flabel08.log,replace
...
//  *
//  log close
//  translate flows_age_bad.smcl flows_age_bad_$freq.log,replace
*
